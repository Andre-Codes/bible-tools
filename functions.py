...
def find_cross_references(df, verse_id, format=None):
    ...
    if format == "clean":
        cross_references = reformat_verse(cross_references)
        main_verse = reformat_verse(main_verse)
...
def reformat_verse(verse):
    verse = verse['Book_Name'] + ' ' + verse['Chapter'].astype(str) + ':' + verse['Verse'].astype(str) + ' ' + verse['origin_text']
